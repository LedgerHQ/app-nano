...
void ed25519_publickey(const libn_private_key_t prv, libn_public_key_t pub) {
    ...
    cx_ecfp_init_private_key_no_throw(CX_CURVE_Ed25519,
                                               ...
                                               &sdkPrv);
    cx_ecfp_init_public_key_no_throw(CX_CURVE_Ed25519, NULL, 0, &sdkPub);

    cx_ecfp_generate_pair2_no_throw(CX_CURVE_Ed25519, &sdkPub, &sdkPrv, true, CX_BLAKE2B);
    memset(&sdkPrv, 0, sizeof(sdkPrv));

    cx_edwards_compress_point_no_throw(CX_CURVE_Ed25519, sdkPub.W, sdkPub.W_len);
...
bool ed25519_sign_open(const uint8_t *m,
    ...
    cx_edwards_decompress_point_no_throw(CX_CURVE_Ed25519, sdkPub.W, sdkPub.W_len);
    sdkPub.W_len = 65;
